...
try {
  ...
  console.log('Failed to load `android/build.gralde` when linking react-native-twilio-programmable-voice');
}
...
if (String(dep).match(/url[^h]*https\:\/\/jitpack\.io/) === null) {
  ...
  dep = String(dep).replace(/repositories[^\{]*\{/, 'repositories {\r\n        // Add jitpack repository (added by react-native-twilio-programmable-voice)\r\n        maven { url "https://jitpack.io" }');
